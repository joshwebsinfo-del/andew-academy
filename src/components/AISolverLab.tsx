...
export default function AISolverLab({
  ...
  const exportToText = () => {
    ...
    let content = `======================================\n`;
    content += `ANDREW ACADEMY: METALLURGICAL LAB NOTES\n`;
    content += `======================================\n\n`;
    content += `TOPIC: ${solution.topic || "Analysis Result"}\n\n`;
    content += `QUESTION:\n${question}\n\n`;
    content += `ABSTRACT SUMMARY:\n${solution.summary}\n\n`;
    
    if (solution.chemicalEquations && solution.chemicalEquations.length > 0) {
      content += `CHEMICAL / PHASE EQUATIONS:\n`;
      solution.chemicalEquations.forEach((eq, idx) => {
        content += ` ${idx + 1}. ${eq.reactants} ${eq.arrow} ${eq.products}\n`;
        content += `    Balanced: ${eq.balanced ? 'Yes' : 'No'}\n`;
        if (eq.notes) content += `    Notes: ${eq.notes}\n`;
      });
      content += `\n`;
    }

    content += `CALCULATED STEPS:\n`;
    solution.steps.forEach((step) => {
      content += ` Step ${step.number}: ${step.title}\n`;
      content += `   Explanation: ${step.explanation}\n`;
      if (step.formula) content += `   Formula: ${step.formula}\n`;
      if (step.calculation) content += `   Calculation: ${step.calculation}\n`;
    ...
    content += `======================================\n`;
    content += `Generated by Andrew Academy VMS\n`;
    content += `======================================\n`;
    ...
    link.href = url;
    link.download = `Andrew_Academy_Notes_${new Date().getTime()}.txt`;
          ...
          {solution && (
            <div className="flex flex-wrap items-center gap-2 mt-2 sm:mt-0">
              ...
              <button
                ...
                className="text-[10px] sm:text-xs px-3 py-1 rounded-full font-bold flex items-center gap-1.5 transition-all text-slate-700 bg-white border border-slate-300 hover:bg-slate-100 shadow-sm select-none shrink-0 cursor-pointer"
                title="Download notes to a text file"
              >
              ...
              <button
                ...
                className={`text-[10px] sm:text-xs px-3 py-1 rounded-full font-bold flex items-center gap-1.5 transition-all text-white select-none shrink-0 cursor-pointer ${
                      ...
                      <div className="space-y-2 flex-grow min-w-0">
                        <div className="flex justify-between items-center gap-2">
                          ...
                          <button
                            ...
                            className={`p-1 rounded-full border transition-all shrink-0 cursor-pointer ${
